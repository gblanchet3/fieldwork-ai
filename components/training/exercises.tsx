  ...
  return (
    <div className="space-y-4">
      ...
      <p className="font-inter text-xs text-steel/70 italic">
        {block.closer ?? "Same model, same ask. The only thing that changed is what it knew about you."}
      </p>
    </div>
  );
}
...
const CANNED_CONVO_FACILITIES = [
  "Happy to help — a few quick questions first:\n\n1. Which floors lose pressure, and does it come and go at certain times of day?\n2. What's the setup — booster pump with a pressure tank, or something else? Roughly how old?\n3. What have you already checked or tried?\n\nAnswer briefly and I'll rank what's most likely. (Offline sample — live, Claude asks about your actual building.)",
  "Here's a first pass:\n\n**Likely causes, most to least likely:**\n1. Waterlogged or ruptured bladder in the pressure tank — the classic \"pressure comes and goes on the upper floors\"\n2. Booster pump short-cycling on a failing pressure switch\n3. A check valve bleeding pressure back when the pump rests\n\n**Checks you can do before the plumber:**\n- Tap down the side of the tank: hollow up top, dull at the bottom is healthy — dull all the way up points at the bladder\n- Watch the gauge while someone runs water upstairs: a fast needle bounce means bladder or switch\n- Listen for the pump kicking on and off every few seconds under light flow — that's the tell\n\nIf it's the bladder, you'll save the diagnostic hour by handing the plumber exactly this. (Offline sample — live, this is built from your answers.)",
  "Tightened into a vendor-ready ticket:\n\n**Work order — domestic water booster system**\nSymptom: Intermittent low pressure, upper floors, worse under light demand.\nChecked: Tank sounds dull well above the waterline; gauge needle bounces under flow; pump short-cycles.\nRequest: Confirm bladder failure, quote tank replacement (or bladder if serviceable) before work.\nPriority: High — tenant-facing.\n\n(Offline sample of a push-back rep.)",
  "One more pass — shorter and firmer:\n\n**Booster system, upper-floor pressure drops.** Tank sounds waterlogged, gauge bounces, pump short-cycles. Need confirmation + replacement quote this week; tenants are noticing. Please confirm an arrival window today.\n\n(Offline sample — two reps in, you can feel it converge.)",
];
...
const CANNED_FACILITIES: CannedSet = {
  contextBare:
    "Work Order\n\nIssue: HVAC unit making noise in office building.\nAction: Technician to inspect and repair as needed.\nPriority: Standard.\nNotes: Please provide additional details regarding the location and nature of the problem.",
  contextRich:
    "**Work order — Suite 410, VAV box (above ceiling)**\n\nSymptom: Rattling from ceiling unit; two tenant complaints this week (second noise issue on this floor this month).\nFound: Worn actuator; play in the damper linkage.\nTemporary fix: Linkage adjusted — quiet for now.\nNeeded: Replace VAV actuator; verify damper linkage while the ceiling is open.\nVendor: Mechanical contractor out this week.\nPriority: High — noise-sensitive tenant (accounting firm, on calls all day).\n\n**Update for the property manager to send:**\n\"Good news — we found the source of the rattle above your suite and quieted it today. The permanent fix (a small replacement part) is scheduled this week; you shouldn't hear a thing in the meantime.\"",
  testWithout:
  ...
  testWith:
    "**Work order — Suite 410, VAV box (above ceiling)**\n\nSymptom: Rattle from the ceiling unit; two tenant complaints this week.\nFound: Worn actuator, play in the damper linkage.\nTemp fix: Linkage adjusted — quiet for now.\nNeeded: Replacement actuator; mechanical vendor out this week.\nPriority: High — noise-sensitive tenant, on calls all day.\nAccess: After 5pm preferred; coordinate with the property manager.\n\n(Notice the difference: the context version reads like someone who knows the building wrote it.)",
  ...ITERATE_CANNED,
};
